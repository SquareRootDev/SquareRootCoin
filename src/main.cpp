...
// Copyright (c) 2011-2013 The SquareRootCoin developers
...
const string strMessageMagic = "SquareRootCoin Signed Message:\n";
...
void static SyncWithWallets(const CTransaction& tx, const CBlock* pblock = NULL, bool fUpdate = false, bool fConnect = true)
{
    if (!fConnect)
    {
        // SquareRootCoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // SquareRootCoin: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // SquareRootCoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// SquareRootCoin: find block wanted by given orphan block
...
int64 GetProofOfStakeReward(int64 nCoinAge, int nHeight)
{
    ...
    int64 nSubsidy = ((sqrt(nCoinAge)) / (sqrt(nHeight))) * COIN; // Square Root of Coinage Divided by Square Root of Block Number is PoS Reward Amount
    return nSubsidy;
}
...
// SquareRootCoin: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // SquareRootCoin: target change every block
    // SquareRootCoin: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // SquareRootCoin: should not enter safe mode for longer invalid chain
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // SquareRootCoin: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // SquareRootCoin: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // SquareRootCoin: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // SquareRootCoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // SquareRootCoin: track money supply and mint amount info
    ...
    // SquareRootCoin: fees are not collected by miners as in bitcoin
    // SquareRootCoin: fees are destroyed to compensate the entire network
...
// SquareRootCoin: total coin age spent in transaction, in the unit of coin-days.
...
// SquareRootCoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // SquareRootCoin: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // SquareRootCoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // SquareRootCoin: record proof-of-stake hash value
    ...
    // SquareRootCoin: compute stake modifier
...
bool CBlock::CheckBlock() const
{
    ...
    // SquareRootCoin: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // SquareRootCoin: coinbase output should be empty if proof-of-stake block
    ...
    // SquareRootCoin: check block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // SquareRootCoin: check that the block satisfies synchronized checkpoint
    ...
    // SquareRootCoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // SquareRootCoin: check proof-of-stake
    ...
    // SquareRootCoin: verify hash target and signature of coinstake tx
    ...
    // SquareRootCoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // SquareRootCoin: getblocks may not obtain the ancestor block rejected
    ...
    // SquareRootCoin: if responsible for sync-checkpoint send it
...
// SquareRootCoin: entropy bit for stake modifier if chosen by modifier
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "SquareRootCoin", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "SquareRootCoin", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // SquareRootCoin: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");

        // SquareRootCoin: upgrade time set to zero if txdb initialized
        {
            CTxDB txdb;
    ...
    // SquareRootCoin: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // SquareRootCoin: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // SquareRootCoin: should not enter safe mode for longer invalid chain
    // SquareRootCoin: if sync-checkpoint is too old do not enter safe mode
    ...
    // SquareRootCoin: if detected invalid checkpoint enter safe mode
    ...
    // SquareRootCoin: if detected unmet upgrade requirement enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // SquareRootCoin: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // SquareRootCoin: record my external IP reported by peer
        ...
        // SquareRootCoin: relay sync-checkpoint
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // SquareRootCoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // SquareRootCoin: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // SquareRootCoin: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // SquareRootCoin: simplify transaction fee - allow free = false
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // SquareRootCoin: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
