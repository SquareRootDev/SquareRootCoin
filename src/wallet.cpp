...
// SquareRootCoin: optional setting to unlock wallet for block minting only;
...
// SquareRootCoin: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // SquareRootCoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // SquareRootCoin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar")) // SquareRootCoin: not avatar mode
                    {
...
// SquareRootCoin: create coin stake transaction
...
// SquareRootCoin: check 'spent' consistency between wallet and txindex
// SquareRootCoin: fix wallet spent state according to txindex
...
// SquareRootCoin: disable transaction (only for coinstake)
