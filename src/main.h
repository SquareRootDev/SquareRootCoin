...
static const int COINBASE_MATURITY_PPC = 8;
...
public:
    ...
    {
        // SquareRootCoin: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    int64 GetMinFee(unsigned int nBlockSize=1, bool fAllowFree=false, enum GetMinFee_mode mode=GMF_BLOCK) const
    {
        ...
        if (fAllowFree)
        {
            if (nBlockSize == 1)
            {
                // Transactions under SquareRootCoin are free
    ...
    bool GetCoinAge(CTxDB& txdb, uint64& nCoinAge) const;  // SquareRootCoin: get transaction coin age
...
public:
    ...
    // SquareRootCoin: block signature - signed by coin base txout[0]'s owner
    ...
    // SquareRootCoin: two types of block: proof-of-work or proof-of-stake
    ...
    // SquareRootCoin: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64& nCoinAge) const; // SquareRootCoin: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // SquareRootCoin: entropy bit for stake modifier if chosen by modifier
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    CBigNum bnChainTrust; // SquareRootCoin: trust score of block chain
    int nHeight;
    int64 nMint;
    int64 nMoneySupply;

    unsigned int nFlags;  // SquareRootCoin: block index flags
    enum  
    {
