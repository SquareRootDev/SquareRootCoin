...
{
    // Wait for IRC to get it first - disabled with SquareRootCoin
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "SquareRootCoin " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
// testnet dns seed begins with 't', all else are SquareRootCoin dns seeds.
static const char *strDNSSeed[][2] = {
    {"seed", "seed.thecryptoninja.website"},
};
...
// SquareRootCoin: stake minter thread
...
bool BindListenPort(string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, sizeof(sockaddr)) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to port %d on this computer.  SquareRootCoin is probably already running."), ntohs(sockaddr.sin_port));
        else
...
void StartNode(void* parg)
{
    ...
    // IRC disabled with SquareRootCoin
    ...
    // SquareRootCoin: mint proof-of-stake blocks in the background
